...
\lstdefinelanguage{jouette}{ 
    keywords={LOAD, SUBI, STORE, ADD, ADDI, MUL, SUB, COPY},
    ndkeywords={fp, r0},
    sensitive=true,
    comment=[l]{\#},
    % morecomment=[s]{/*}{*/},
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}
        ...
        \draw[dotted] (0, 1) -- (1, 1);
        \node at (1.5, 0.5) {\texttt{A[0]}};

        \draw[dotted] (0, 2) -- (1, 2);
        \node at (1.5, 1.5) {\texttt{A[1]}};
        
        \draw[dotted] (0, 3) -- (1, 3);
        \node at (1.5, 2.5) {\texttt{A[2]}};
    \end{tikzpicture}
\end{figure}
...
Within the BB, we determine where each variable is alive and then allocate registers. For the BB above, we determine the register allocation and allocate registers as follows.
\begin{figure}[H]
    \centering
    \begin{tikzpicture}
...
\begin{figure}[H]
    \centering
    \begin{tikzpicture}
        \draw (0, 0) node[label=120:BB1] {} rectangle (2, -1.5);
        \node[align=left] at (0.75, -0.75) {p $\leftarrow$ 1 \\ q $\leftarrow$ b \\ m $\leftarrow$ n};

        \draw (0, -2.5) node[label=90:BB2] {} rectangle (2, -3.5);
        \node[align=left] at (1, -3) {t1 $\leftarrow$ m$>$0 \\ jump if t1};

        \draw (1, -4.5) node[label=90:BB3] {} rectangle (3, -5.5);
        \node[align=left] at (2, -5) {t2 $\leftarrow$ m$\&$1 \\ jump if t2};

        \draw (2, -6.5) node[label=90:BB4] {} rectangle (4, -7);
        \node[align=left] at (3, -6.75) {p $\leftarrow$ p*q};

        \draw (1, -8) node[label=90:BB5] {} rectangle (3, -9);
        \node[align=left] at (2, -8.5) {m $\leftarrow$ m/2 \\ q $\leftarrow$ q*q};

        \draw (0, -10) node[label=90:BB6] {} rectangle (2, -10.5);
        \node[align=left] at (0.75, -10.25) {return p};

        \draw[->] (1, -1.5) -- (1, -2.5);
        \draw[->] (1, -3.5) -- (2, -4.5);
        \draw[->] (2, -5.5) -- (3, -6.5);
        \draw[->] (3, -7) -- (2, -8);
        \draw[->] (3, -8.5) -- (5, -8.5) -- (5, -3) -- (2, -3);
        \draw[->] (0, -3) -- (-1, -3) -- (-1, -10.25) -- (0, -10.25);
    \end{tikzpicture}
\end{figure}
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        \node[ellipse, draw] {move}
        child {
            node[ellipse, draw] {mem}
            child {
                node[ellipse, draw] {+}
                child {
                    node[ellipse, draw] {mem}
                    child {
                        node[ellipse, draw] {+}
                        child {
                            node[draw, label=-90:REG] {fp}
                        }
                        child {
                            node[draw, label=-90:CONST] {$a$}
                        }
                    }
                }
                child {
                    node[ellipse, draw] {$\times$}
                    child {
                        node[draw, label=-90:REG] {9}
                    }
                    child {
                        node[draw, label=-90:CONST] {4}
                    }
                }
            }
        }
