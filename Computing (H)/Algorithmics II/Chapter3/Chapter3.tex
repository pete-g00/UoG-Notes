...
\end{lstlisting}
    ...
    \begin{itemize}
        ...
        \item $v_1$ has an edge to $v_2$ of shortest weight, so $v_1 - v_2$ has weight $4$.
    \end{itemize}
    However, it is possible to take the path $v_1 - v_3 - v_2$ to get a path of weight $1$. We cannot find this since the algorithm is greedy, which only works if the weights do cannot be decreased.
